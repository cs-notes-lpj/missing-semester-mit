...
6、Look into what the `-N` and `-f` flags do in `ssh` and figure out a command to achieve background port forwarding.

```shell
ssh -fN -L 9999:localhost:8888 myServerIP
```
...
1、learn to use [mosh](https://mosh.org/).

2、read and practise [this](https://dotfiles.github.io/) to get more knowledge relevant to dotfiles.
