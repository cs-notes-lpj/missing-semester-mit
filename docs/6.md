...
8、`git bisect`, not only interactive, but also can take in a script that is used to try to figure out whether a commit looking at is good or bad, so it can be fully automated process like you can give git-bisect a unit test and say:"find the first commit where this unit test stopped passing."
